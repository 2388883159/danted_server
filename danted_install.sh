...
if (([ -f "/etc/issue" ] && [ -n "$(grep -E 'CentOS|Oracle' /etc/issue)" ]) || 
    ([ -f "/etc/centos-release" ] && [ -n "$(grep CentOS /etc/centos-release)" ]) || 
    ([ -f "/etc/oracle-release" ] && [ -n "$(grep Oracle /etc/oracle-release)" ])); then
    yum install gcc g++ make vim pam-devel tcp_wrappers-devel unzip httpd-tools wget net-tools -y
fi

if ([ -n "$(grep -E 'Debian|Ubuntu' /etc/issue)" ]); then
    echo "ubuntu system"
    apt-get update
    apt-get install gcc g++ make vim libpam-dev libwrap0-dev unzip apache2-utils wget net-tools -y
fi
...
wget --no-check-certificate https://github.com/qingtengyun/danted_server/raw/master/dante-1.4.1.tar.gz
tar zxvf dante*
cd dante*
...
[ -n "$(grep -E 'CentOS|Oracle' /etc/issue)" ]  && chkconfig --add danted
